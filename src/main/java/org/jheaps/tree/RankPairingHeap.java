...
public class RankPairingHeap<K, V> implements MergeableAddressableHeap<K, V>, Serializable {
	...
	public AddressableHeap.Handle<K, V> deleteMin() {
		...
		Node<K, V> spine = severSpine(minRoot.l);
		minRoot.l = null;
		...
		/*
		 * One pass old half-trees, careful to skip old minimum which is still in the
		 * root list.
		 */
		while (minRoot != null) {
			// remove second from list
	...
	/*
	 * Decrease the key of a node to the minimum. Helper function for performing a
	 * delete operation. Does not change the node's actual key, but behaves as the
	 * key is the minimum key in the heap.
	 */
	...
	/*
	 * Iterate over all right children of x and create a list of half-tree roots. We
	 * only need to nullify the parent pointers. At the same time give each new root
	 * a rank that is one greater than that of its left child.
	 */
	...
	/*
	 * Cut x and its left child from its parent. Leave the right child of x in place
	 * of x.
	 */
